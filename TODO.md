...
- [x] 1.1 Crear `frontend/app/layouts/AreaLayout.vue`: header con breadcrumb "← Composer" + título/icono, slots `sidebar` y `editor`, responsive drawer en móvil
- [x] 1.2 Crear `frontend/app/pages/character.vue` usando `AreaLayout` + `CharacterEditor` (sin AdvancedPanel, se añade en Fase 1)
- [x] 1.3 Crear `frontend/app/pages/outfit.vue` usando `AreaLayout` + `OutfitEditor` + sidebar con botón catálogo (modal GarmentPicker)
- [x] 1.4 Crear `frontend/app/pages/pose.vue` usando `AreaLayout` + `PoseEditor`
- [x] 1.5 Crear `frontend/app/pages/scene.vue` usando `AreaLayout` + `SceneEditor`
- [x] 1.6 Crear `frontend/app/pages/time.vue` usando `AreaLayout` + `TimeWeatherEditor`
- [x] 1.7 Crear `frontend/app/pages/lighting.vue` usando `AreaLayout` + `LightingEditor`
- [ ] 1.8 Añadir alias `/composer` a `pages/index.vue` (deferred: dashboard vive en `/dashboard`)

## 2. Frontend — Navegación desde dashboard
- [x] 2.1 `BlockSidebar.vue`: añadir botón expand (⤢) junto a cada toggle que navega a `/${blockKey}`
- [x] 2.2 `BlockEditor.vue`: botón "Editar en profundidad" → navega a `/${blockKey}`
- [x] 2.3 `AreaLayout.vue`: listener global `Esc` → `navigateTo('/dashboard')`

## 3. Frontend — Sincronización y AdvancedPanel persistente
- [ ] 3.1 Verificar que `AdvancedPanel` en rutas dedicadas usa mismo `dashboardStore.getModifiers/setModifiers` (pendiente Fase 1)
- [ ] 3.2 En `AreaLayout`, `AdvancedPanel` siempre visible (no dentro de `UiAccordion`); prop `inline=true` para estilo sin acordeón (pendiente Fase 1)
- [ ] 3.3 `GarmentPicker` versión integrada (no modal): componente `GarmentPickerInline.vue` para slot sidebar en `/outfit` (usando modal actual por ahora)

## 4. Frontend — Sidebar específica por área
- [x] 4.1 Sidebar básica reutilizable `AreaSidebar.vue`: lista guardados + Cargar/Guardar/Eliminar + botón catálogo (outfit)
- [ ] 4.2 `CharacterSidebar.vue`, `PoseSidebar.vue`, `SceneSidebar.vue`, `TimeSidebar.vue`, `LightingSidebar.vue`: asset search específico (deferred)

## 5. Verificación
- [x] 5.1 `npx nuxi typecheck` — sin errores (solo WARN preexistente)
- [x] 5.2 Smoke manual: login → `/dashboard` → click "Editar en profundidad" en Outfit → `/outfit` abre con editor completo + sidebar con catálogo → cambiar prenda → volver a `/dashboard` → outfit actualizado en BlockEditor → compilar → refleja cambios → navegar `/character` → editar → volver → sincronizado
